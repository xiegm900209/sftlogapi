...
def test_log_parsing():
    ...
    if os.path.exists(sample_log_path):
        print(f"解析日志文件: {sample_log_path}")
        ...
        for i, block in enumerate(read_log_blocks(sample_log_path)):
            ...
            if i < 3:  # 只打印前3个日志块作为示例
                ...
                print(f"  TraceID: {block.trace_id}")
                print(f"  服务: {block.service}")
                print(f"  级别: {block.level}")
                # 处理内容中的特殊字符以避免编码错误
                content_preview = block.content[:100].encode('utf-8', errors='ignore').decode('utf-8', errors='ignore')
                print(f"  内容: {content_preview}...")  # 只显示前100个字符
                print()

            if i >= 9:  # 只处理前10个块用于测试
                break

        print(f"总共解析了 {len(log_blocks)} 个日志块\n")
    else:
        print(f"示例日志文件不存在: {sample_log_path}")
        # 尝试另一种路径
        alt_path = "D:/sft-log/testlogs/testlogs/sft-aipg/sft-aipg-sft-aipg-59c947b9c9-cj6fm_zb_2026040809.log"
        if os.path.exists(alt_path):
            print(f"发现日志文件在: {alt_path}")
...
def test_req_sn_search():
    ...
    print(f"搜索REQ_SN: {req_sn}")
    logs = find_logs_by_req_sn('sft-aipg', req_sn, '../../testlogs/testlogs')

    if logs:
        print(f"找到 {len(logs)} 条相关日志:")
        for i, log in enumerate(logs[:3]):  # 只显示前3条
            print(f"  {i+1}. TraceID: {log.trace_id}, 时间: {log.timestamp}")
            content_preview = log.content[:100].encode('utf-8', errors='ignore').decode('utf-8', errors='ignore')
...
def test_index_builder():
    ...
    try:
        ...
        print(f"构建完成，找到 {len(index)} 个唯一的TraceID")

        # 显示前几个TraceID作为示例
        trace_ids = list(index.keys())[:5]
        for trace_id in trace_ids:
            entries = index[trace_id]
            print(f"  {trace_id}: {len(entries)} 个日志条目")
...
def test_trace_analyzer():
    ...
    try:
        ...
        if result['success']:
            ...
            print(f"  交易类型: {result['transaction_type'] or '未指定'}")
            print(f"  日志数量: {len(result['trace_data'])}")

            # 显示前几个日志条目的摘要
            for i, log_item in enumerate(result['trace_data'][:3]):
                print(f"  {i+1}. {log_item['timestamp']} - {log_item['app']} - {log_item['level']}")
        else:
